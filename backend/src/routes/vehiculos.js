...
router.post('/', requireAuth, requireRole('admin', 'supervisor'), async (req, res) => {
  const { placa, marca, modelo, anio, tipo, color, dependencia_id, combustible } = req.body
  if (!placa || !marca || !modelo) return res.status(400).json({ error: 'Faltan campos requeridos' })

  const { rows } = await db.query(`
    INSERT INTO vehiculos (placa, marca, modelo, anio, tipo, color, dependencia_id, combustible)
...
router.put('/:id', requireAuth, requireRole('admin', 'supervisor'), async (req, res) => {
  ...
  `, [placa, marca, modelo, anio ?? null, tipo ?? null, color ?? '', dependencia_id ?? null, combustible ?? 'gasolina', req.params.id])
